...
parameter int MIN_X_SPEED = 40;
parameter int MAX_X_SPEED = 100;
...
//					Yspeed <= -Yspeed;//+1 ; 
//			if((Xspeed <= MAX_X_SPEED && Xspeed >= MIN_X_SPEED) ||
//				(Xspeed >= -MAX_X_SPEED && Xspeed <= -MIN_X_SPEED)) begin 
				if (accel_right_key & !accel_right_key_D) //rizing edge 
						Xspeed <= Xspeed-X_ACCEL; // toggle direction 
				if (accel_left_key & !accel_left_key_D) //rizing edge 
						Xspeed <= Xspeed+X_ACCEL; // toggle direction
//			end
